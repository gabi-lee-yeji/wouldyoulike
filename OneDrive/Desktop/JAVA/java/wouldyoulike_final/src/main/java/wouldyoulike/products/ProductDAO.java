...
public class ProductDAO {
	...
	public ArrayList<ProductDTO> filtering (ProductDTO dto){
		ArrayList<ProductDTO> list = new ArrayList<ProductDTO>();
		try {
			conn = OracleConnection.getConnection();
			int filterCnt = 0;
			...
			if(dto.getCategory() != null) {
				...
				sql += " where procategory in ( "+category+" ) ";
			}
			...
			if(dto.getMinPrice()!=0 || dto.getMaxPrice()!=0) {
				...
				if(filterCnt == 1) {
					sql += " where price between "+dto.getMinPrice()+" and "+maxPrice+" ";
				}else {
					sql += " and price between "+dto.getMinPrice()+" and "+maxPrice+" ";
				}	
			}
			...
			if(dto.getMinSize()!=0 || dto.getMaxSize() !=0) {
				...
				if(filterCnt == 1) {
					sql += " where winesize between "+dto.getMinSize()+" and "+maxSize+" ";
				}else {
					sql += " and winesize between "+dto.getMinSize()+" and "+maxSize+" ";
				}
			}
			...
			if(dto.getMaxAbv()!=0){
				filterCnt += 1;
				if(filterCnt == 1) {
					sql += " where abv <= "+dto.getMaxAbv()+" ";
				}else {
					sql += " and abv <= "+dto.getMaxAbv()+" ";
				}
			}
			...
			if(dto.getRating()!=0) {
				filterCnt += 1;
				if(filterCnt ==1 ) {
					sql += " where rating >= "+dto.getRating()+" ";
				}else {
					sql += " and rating >= "+dto.getRating()+" ";
				}
			}
			System.out.println(sql);
			System.out.println(filterCnt);
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next()) {
